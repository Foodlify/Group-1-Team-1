...
export class OrderService {
  static async getOrderStatus(
    ...
    loggerService.info('Getting order status for payment type', { paymentName });
    let orderStatus;
    if (paymentName === PaymentTypeEnum.CASH) {
      orderStatus = await OrderRepository.getOrderStatusByName(OrderStatusEnum.CONFIRMED, db);
    } else {
      orderStatus = await OrderRepository.getOrderStatusByName(OrderStatusEnum.PENDING, db);
    }
    if (!orderStatus) {
      loggerService.warn('Order status not found', { paymentName });
      throw new NOT_FOUND(ENTITIES.ORDER_STATUS);
    }
    loggerService.info('Order status resolved', { paymentName, statusId: orderStatus.id });
  ...
  static async placeOrder(input: CreateOrderInput): Promise<any> {
    ...
    loggerService.info('Place order attempt', { customerId, addressId, paymentTypeId });
    ...
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      try {
        ...
        loggerService.info('Order placed successfully', { customerId, addressId, paymentTypeId });
        return result;
      } catch (error) {
        loggerService.error('Place order failed', error as Error, { customerId, addressId, paymentTypeId });
        throw error;
      }
    });
  }
  ...
  static async checkout(
    ...
    if (!redisCart || redisCart.items.length === 0) {
      loggerService.warn('Checkout failed: cart empty or not found', { customerId });
    ...
    if (menuItems.length !== items.length) {
      loggerService.warn('Checkout failed: one or more menu items not found', { customerId });
    ...
    for (const item of items) {
      ...
      if (menuItem.restaurantId !== restaurantId) {
        loggerService.warn('Checkout failed: restaurant mismatch', { customerId, menuItemId: item.menuItemId });
        throw new RestaurantNotMatch(errorMessage.RESTAURANT_NOT_MATCH.message);
      }
      if (item.quantity > menuItem.stock) {
        loggerService.warn('Checkout failed: quantity exceeds stock', { customerId, menuItemId: item.menuItemId, quantity: item.quantity, stock: menuItem.stock });
        throw new QuantityExceed(errorMessage.QUANTITY_EXCEED.message);
      }
      if (Number(menuItem.price) !== item.price) {
        loggerService.warn('Checkout failed: price changed', { customerId, menuItemId: item.menuItemId, cartPrice: item.price, currentPrice: menuItem.price });
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (existingCart) {
        if (existingCart.isLocked) {
          loggerService.warn('Checkout failed: cart is locked', { customerId, cartId: existingCart.id });
      ...
      await CartRepository.createCartWithManyItems(customerId, restaurantId, cartItemsToInsert, tx);

      loggerService.info('Checkout completed', { customerId, restaurantId, itemsCount: items.length });
      return { customerId, restaurantId, itemsCount: items.length, cartItems: cartItemsToInsert };
    });
  }
  ...
  static async getSingleOrder(
    ...
    const order = await OrderRepository.getSingleOrderById(customerId, orderId, db);
    if (!order) {
      loggerService.warn('Order not found', { customerId, orderId });
      throw new NOT_FOUND(ENTITIES.ORDER);
    }
    const result = await OrderRepository.getSingleOrderAndDetailsById(orderId, db);
    if (!result) {
  ...
  static async updateOrderStatus(
    ...
    loggerService.info('Update order status', { customerId, orderId, newStatus });

    const order = await OrderRepository.getSingleOrderById(customerId, orderId, db);
    if (!order) {
      loggerService.warn('Update order status failed: order not found', { customerId, orderId });
      throw new NOT_FOUND(ENTITIES.ORDER);
    }

    const currentStatusEntity = await OrderRepository.getOrderStatusById(order.orderStatusId, db);
    if (!currentStatusEntity) {
      loggerService.warn('Update order status failed: current status not found', { orderId, orderStatusId: order.orderStatusId });
    ...
    switch (newStatus) {
      ...
      case OrderStatusEnum.REFUNDED:
        ...
        break;
      default:
        loggerService.warn('Update order status failed: invalid transition', { orderId, newStatus });
    ...
    const newStatusEntity = await OrderRepository.getOrderStatusByName(resolvedStatus, db);
    if (newStatusEntity) {
      await OrderTrackingService.addOrderTrackingStatus(orderId, newStatusEntity.id, createdBy, db);
    }

    loggerService.info('Order status updated', { customerId, orderId, from: currentStatusEntity.name, to: resolvedStatus });
  }
  ...
  static async getOrdersByStatus(
    ...
    const orders = await OrderRepository.getOrdersByCustomerAndOrderStatus(customerId, status);
    loggerService.info('Orders retrieved by status', { customerId, status, count: orders.length });
  ...
  private static async insertOrderSummaryTrigger(
    ...
    loggerService.info('Inserting order summary trigger', { customerId, orderId });
    ...
    await OrderSummaryService.addOrderSummary({
      ...
      totalQuantity,
      orderDate:
        orderDetails.date instanceof Date ? orderDetails.date : new Date(orderDetails.date),
    });

    loggerService.info('Order summary inserted', { customerId, orderId, totalQuantity });
  }
}
