...
export class CustomerController {
  ...
  login = asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await CustomerService.login(req.body, meta);
  ...
  refreshToken = asyncHandler(async (req: Request, res: Response) => {
    ...
    const meta   = { ip: req.ip, deviceInfo: req.headers['user-agent'] };
    const result = await CustomerService.refreshToken({ refreshToken }, meta);
    setAuthCookies(res, result.accessToken, result.refreshToken);
    sendSuccess(res, successMessage.TOKEN_REFRESHED.message, StatusCodes.OK);
  });

  logout = asyncHandler(async (req: Request, res: Response) => {
    await CustomerService.logout(req.userId!, req.cookies?.refreshToken);
